...
\[\]
Ara, per trobar generadors de l'ideal, notem que $I_y=(y-x^2)$ i $I_z(z-x^3)$ estan a $I(Y)$, per veure que generen l'ideal, construirem una seqüència d'irreductibles, però primer notem que per (1.8A), tenim que:
\[
...
\newpage
\begin{tcolorbox}[title=\subsection{Exercici 2.2}]
    For a homogeneous ideal $\mfrk a \subseteq S$, show that the following conditions are equivalent:
    \begin{enumerate}
        \item $Z(\mfrk a)=\varnothing$.
        \item $\sqrt{\mfrk a} = $ either $S$ or the ideal $S_+=\bigoplus_{d>0}S_d$.
        \item $\mfrk a\supseteq S_d$ for some $d>0$.
    \end{enumerate}
\end{tcolorbox}
\subsubsection{Exercici 2.2}
\textbf{i) $\implies$ ii)}\\
We want to use Hilbert's Nullstellensatz, which says that if we have $f\in S_+$ such that $f(p)=0$ for all $p\in Z(\mfrk a)$ (which is vacuously true), we have that $f\in \sqrt{\mfrk a}$. So we have that $S_+\subseteq \sqrt{\mfrk a}$; but the only ideal that contain $S_+$ are $S$ and $S_+$.\par
\textbf{ii) $\implies$ iii)}\\
Note that $x^{n_1}\in \mfrk a$ for some $n_1$ and that $y^{n_2}\in \mfrk a$ for some $n_2$. Let us consider $n>n_1,n_2$ then $x^n\in \mfrk a$ and $y^n\in \mfrk a$.\\
Now let us consider $d=2n$, then note that in $x^iy^j$ we have either $i>n$ or $j>n$, thus either $x^i \in \mfrk a$ or $x^j \in \mfrk a$; so we have that $S_d\subseteq \mfrk a$ (this is the case of 2 variables, but this can be easily).\par
Note that if $x_i^d\in \mfrk a$, then $x_i=0$ for all $i$.
\newpage
\begin{tcolorbox}[title=\subsection{Exercici 2.3}]
    \begin{enumerate}
        \item If $T_1\subseteq T_2$ are subsets of $S^h$ then $Z(T_1)\supseteq T_2$.
        \item If $Y_1\subseteq Y_2$ are subsets of $\PP^n$, then $I(Y_1)\supseteq I(Y_2)$.
        \item For any two subsets $Y_1$, $Y_2$ of $\PP^n$, $I(Y_1\cup Y_2)=I(Y_1)\cap I(Y_2)$.
        \item If $\mfrk a\subseteq S$ is a homogeneous ideal with $Z(\mfrk a)\neq \varnothing$, then $I(Z(\mfrk a))=\sqrt{\mfrk a}$.
        \item For any subset $Y\subseteq \PP^n$, $Z(I(Y))=\ol Y$.
    \end{enumerate}
\end{tcolorbox}
\subsubsection{Exercici 2.3}
Note that the first 2 are true by definition.\par
Let $Y_1$ and $Y_2$ be points of $\PP^n$, then $I(Y_1\cup Y_2)$ are the polynomials that annul $Y_1$ and $Y_2$ which is $I(Y_1)\cap I(Y_2)$.\par
This is comes directly from applying the \textit{homogeneous Nullstellensatz}.\par
Since $Z$ is the set of points that annul every polynomial of $I(Y)$, it must be the smallest variety that contains $Y$.

\newpage
\begin{tcolorbox}[title=\subsection{Exercici 2.4}]
    \begin{enumerate}
        \item There is a 1-1 inclusion-reversing correspondence between algebraic sets in $\PP^n$ and homogeneous radical ideals of $S$ not equal to $S_+$, given by $Y\mapsto I(Y)$ and $\mfrk a\mapsto Z(\mfrk a)$. \textit{Note: Since $S_+$ does not occur in this correspondence, it is sometimes calles the irrelevant maximal ideal of $S$.}
        \item An algebraic set $Y\subseteq \PP^n$ is irreducible iff $I(Y)$ is a prime ideal.
        \item Show that $\PP^n$ itself is irreducible.
    \end{enumerate}
\end{tcolorbox}
\subsubsection{Exercici 2.4.1}
We already know that it is inclusion reversing, so all we need to show is that it's bijective. Let us consider $X$, and $Y$ such that $I(X)=I(Y)$ then since $\ol X=Z(I(X))=Z(I(Y))=\ol Y$, and since they are varieties, we are done. To see that it's surjective, let us notice that if $Z$ is an homogeneous radical ideal of $S$ not equal to $S_+$, then $I(Z(Z))=Z$, meaning, that it's in the image of $I$.
\subsubsection{Exercici 2.4.2}
Remember that a variety is irreducible if there is no proper closed subset that contains it. Thus, by what we've just proven, we get that 
